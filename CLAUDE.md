...
- **`src/app/globals.css` is the source of truth for token values** — a port of the DS `tokens/*.css` remapped onto the shadcn token names, with one deliberate override (2026-08-05): the warm sand/stone/ink neutrals are replaced by the user's **Ivory & Forest neutrals** (`#fcfcfa` page, `#f4f4ef` sunken, `#e8e8e0`/`#d6d6cb` borders, `#a1a199`/`#767670`/`#545450` text grays, `#2d2e2b`/`#1a1b19` ink). Greens/accents stay DS. No confetti anywhere (user decision — celebrations module removed).
